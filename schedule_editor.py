...
class ScheduleEditorWindow(customtkinter.CTkToplevel):
    def __init__(self, parent_app):
        ...
        # Day selector frame
        day_selector_frame = customtkinter.CTkFrame(main_container, fg_color="transparent")
        day_selector_frame.grid(row=0, column=0, columnspan=2, sticky="ew", padx=10, pady=5)
        ...
        self.editor_frame = customtkinter.CTkScrollableFrame(main_container, label_text="Програма за избрания ден", height=300)
        self.editor_frame.grid(row=1, column=0, columnspan=2, sticky="nsew", padx=10, pady=10)
        self.editor_frame.grid_columnconfigure(0, weight=1)

        # Add entry frame
        add_frame = customtkinter.CTkFrame(main_container)
        add_frame.grid(row=2, column=0, columnspan=2, sticky="ew", padx=10, pady=10)
        ...
        # Bulk edit section
        self.setup_bulk_edit_frame(main_container)
        ...
        # The day selection is bound to the command callback in the option menu

    def setup_bulk_edit_frame(self, parent_container):
        bulk_edit_frame = customtkinter.CTkFrame(parent_container, fg_color="transparent")
        bulk_edit_frame.grid(row=3, column=0, columnspan=2, sticky="ew", padx=10, pady=10)
        ...
        source_frame.grid(row=1, column=0, columnspan=3, sticky="ew", padx=5, pady=5)
        source_frame.grid_columnconfigure(1, weight=1)
        
        customtkinter.CTkLabel(source_frame, text="Копирай от:").grid(row=0, column=0, sticky="w", padx=(0, 5))
        self.bulk_source_day_var = customtkinter.StringVar(value=DAYS_OF_WEEK[0])
        customtkinter.CTkOptionMenu(source_frame, variable=self.bulk_source_day_var, values=DAYS_OF_WEEK, width=120).grid(row=0, column=1, sticky="w")

        # Target days selection
        target_frame = customtkinter.CTkFrame(bulk_edit_frame, fg_color="transparent")
        target_frame.grid(row=2, column=0, columnspan=3, sticky="ew", padx=5, pady=5)
        
        customtkinter.CTkLabel(target_frame, text="Копирай в:").grid(row=0, column=0, sticky="w", padx=(0, 5), pady=(5,0))
        ...
        # Copy button
        copy_button = customtkinter.CTkButton(bulk_edit_frame, text="Копирай", width=100, command=self.bulk_copy_schedule)
        copy_button.grid(row=3, column=0, columnspan=3, sticky="ew", pady=(10,0))
